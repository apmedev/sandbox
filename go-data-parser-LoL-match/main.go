...
type LeagueOfLegendsUserMatch struct {
	...
	ChallengeID   int //points to challenge id in database
	GameStartTime int64
	GameLength    int
...
func main(){
	const addr = "postgresql://maxroach@localhost:26257/tx_database?sslmode=disable"
	...
	//Important: missing challange_id, user_id and user_challange_id
...
func activeMatchExists(gameid int64, summonerName string)  bool{
	const addr = "postgresql://maxroach@localhost:/s_database?sslmode=disable"
	db, err := gorm.Open("postgres", addr)
	if err != nil {
		log.Fatal(err)
	}
